...
const NAV=[
  ...
  {id:"fx",      icon:"$",  label:"FX Rates",            group:"Setup"},
  {id:"sales",   icon:"◉",  label:"Sales",               group:"Operations"},
  {id:"arap",    icon:"↕",  label:"AR / AP",             group:"Operations"},
  {id:"wc",      icon:"⟳",  label:"Working Capital",     group:"Operations"},
  {id:"forecast", icon:"📅",  label:"13-Week Forecast",    group:"Operations"},
  {id:"close",    icon:"☑",  label:"Month-End Close",     group:"Operations"},
  {id:"vendors",  icon:"🏢",  label:"Vendor Master",       group:"Procure-to-Pay"},
  {id:"pr",       icon:"📋",  label:"Purchase Requisition",group:"Procure-to-Pay"},
  {id:"po",       icon:"📦",  label:"Purchase Orders",      group:"Procure-to-Pay"},
  {id:"gr",       icon:"✅",  label:"Goods Receipt",        group:"Procure-to-Pay"},
  {id:"sinvoice",  icon:"🧾",  label:"Supplier Invoice",     group:"Procure-to-Pay"},
  {id:"payrun",    icon:"💳",  label:"Payment Run",          group:"Procure-to-Pay"},
  {id:"gl",      icon:"⊟",  label:"GL Ledger",           group:"Accounting"},
  {id:"ic",      icon:"⇄",  label:"IC & Consolidation",  group:"Accounting"},
  {id:"pl",      icon:"▤",  label:"P&L",                 group:"Reporting"},
  {id:"bs",      icon:"▥",  label:"Balance Sheet",       group:"Reporting"},
  {id:"budget",  icon:"◎",  label:"Budget vs Actual",    group:"Reporting"},
  {id:"cashflow",icon:"⇌",  label:"Cash Flow",           group:"Reporting"},
  {id:"assets",  icon:"🏗",  label:"Fixed Assets",        group:"Reporting"},
  {id:"headcount",icon:"👥",  label:"Headcount & Payroll", group:"Reporting"},
  {id:"reportpack",icon:"📄", label:"Report Pack",         group:"Reporting"},
  {id:"ai",      icon:"✦",  label:"AI Biz Insight",  group:"Reporting"},
];
...
function PLModule({gf}){
  ...
  function buildPL(forPeriods,entityIds){
    ...
    const cogsAccts=coa.filter(a=>a.class==="X"&&a.active&&["Cost of Sales"].includes(a.name));
    const rows=[];
    let totalRev=0,totalCOGS=0,totalExp=0;

    // Revenue
    revAccts.forEach(acct=>{
      const{dr,cr,net}=getGL(acct.code,forPeriods,entityIds);
      const val=cr-dr; // credit-normal for revenue
      if(Math.abs(val)>0.01){rows.push({type:"R",code:acct.code,name:acct.name,group:acct.group,value:val,isIC:acct.icEligible});totalRev+=val;}
    });

    // COGS
    const cogsAccts2=coa.filter(a=>a.class==="X"&&a.active&&a.code==="5000");
    cogsAccts2.forEach(acct=>{
      const{dr,cr,net}=getGL(acct.code,forPeriods,entityIds);
      const val=dr-cr;
      if(Math.abs(val)>0.01){rows.push({type:"COGS",code:acct.code,name:acct.name,group:acct.group,value:val,isIC:acct.icEligible});totalCOGS+=val;}
    });

    const grossProfit=totalRev-totalCOGS;

    // Opex (exclude COGS)
    expAccts.filter(a=>a.code!=="5000").forEach(acct=>{
      const{dr,cr,net}=getGL(acct.code,forPeriods,entityIds);
      const val=dr-cr;
      if(Math.abs(val)>0.01){rows.push({type:"X",code:acct.code,name:acct.name,group:acct.group,value:val,isIC:acct.icEligible});totalExp+=val;}
    });

    const ebitda=grossProfit-expAccts.filter(a=>["5300"].includes(a.code)).reduce((s,a)=>{const{dr,cr}=getGL(a.code,forPeriods,entityIds);return s+(dr-cr);},0);
  ...
  function PLHead({label,color}){
    return <tr><td colSpan={5} style={{padding:"10px 10px 5px",color:color||P.gold,fontSize:10,fontWeight:700,letterSpacing:1,background:P.surf2,borderTop:`1px solid ${P.border}`}}>{label}</td></tr>;
  }
        ...
        <Card noPad title={`P&L — ${viewMode==="ytd"?"YTD to":""} ${cP2} vs ${cP1} · ${activeE.length} entities · MYR`}>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
              ...
              <tbody>
                <PLHead label="REVENUE" color={P.green}/>
                {pl2.rows.filter(r=>r.type==="R").map(r=>(
                ...
                {pl2.rows.filter(r=>r.type==="COGS").length>0&&<>
                  <PLHead label="COST OF SALES" color={P.orange}/>
                  {pl2.rows.filter(r=>r.type==="COGS").map(r=>(
                ...
                <PLHead label="OPERATING EXPENSES" color={P.red}/>
                {pl2.rows.filter(r=>r.type==="X").map(r=>(
                  <PLRow key={r.code} label={r.name} value={r.value} prevVal={pl1.rows.find(x=>x.code===r.code)?.value} indent sub={r.isIC?"(IC)":null}/>
                ))}
...
function BSModule({gf}){
  ...
  function BSHead({label,color}){
    return <tr><td colSpan={4} style={{padding:"10px 10px 5px",color:color||P.gold,fontSize:10,fontWeight:700,letterSpacing:1,background:P.surf2,borderTop:`1px solid ${P.border}`}}>{label}</td></tr>;
  }
        ...
        <Card noPad title={`Balance Sheet as at ${cP2} vs ${cP1} · ${activeE.length} entities · MYR (cumulative GL balances)`}>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
              ...
              <tbody>
                {/* ASSETS */}
                <BSHead label="ASSETS" color={P.blue}/>
                {(bs2.sections["A"]||[]).map(r=><BSRow key={r.code} label={r.name} value={r.value} prevVal={bs1.sections["A"]?.find(x=>x.code===r.code)?.value} indent sub={r.isIC?"(IC)":null}/>)}
                <BSTotal label="TOTAL ASSETS" value={bs2.totalAssets} prevVal={bs1.totalAssets} color={P.blue}/>

                {/* LIABILITIES */}
                <BSHead label="LIABILITIES" color={P.orange}/>
                {(bs2.sections["L"]||[]).map(r=><BSRow key={r.code} label={r.name} value={r.value} prevVal={bs1.sections["L"]?.find(x=>x.code===r.code)?.value} indent sub={r.isIC?"(IC)":null}/>)}
                <BSTotal label="TOTAL LIABILITIES" value={bs2.totalLiab} prevVal={bs1.totalLiab} color={P.orange}/>

                {/* EQUITY */}
                <BSHead label="EQUITY" color={P.purple}/>
                {(bs2.sections["E"]||[]).map(r=><BSRow key={r.code} label={r.name} value={r.value} prevVal={bs1.sections["E"]?.find(x=>x.code===r.code)?.value} indent/>)}
                {/* Auto-injected retained earnings from P&L */}
                <BSRow label="Retained Earnings (Current Period)" value={bs2.currentNP} prevVal={bs1.currentNP} indent color={bs2.currentNP>=0?P.green:P.red} sub="← auto-linked from P&L"/>
...
function AuthedApp({session,onLogout}){
  ...
  const allowedIds=isAdmin?ADMIN_MODULES:CLIENT_MODULES;
  const visibleNav=NAV.filter(n=>allowedIds.includes(n.id));
  if(isAdmin&&!visibleNav.find(n=>n.id==="users")){
    visibleNav.push({id:"users",icon:"👤",label:"User Management",group:"Admin"});
  }
...
function AIInsightsModule({ gf }) {
  ...
  function buildSystem(context) {
    return "You are a senior Finance Manager and business analyst for a multi-entity APAC business using the FinFlow financial platform by SynerGrowth Consulting. You have deep expertise in management accounting, financial analysis, and business strategy across Southeast Asia — particularly Malaysia, Singapore, and the Philippines.\n\n" +
      "Your communication style is:\n" +
      "- Direct and confident — like a CFO briefing a board\n" +
      "- Specific — always cite actual numbers, entity names, product names from the data\n" +
      "- Actionable — every insight leads to a recommended action\n" +
      "- Concise — no padding, no generic statements\n" +
      "- APAC-aware — sensitive to regional business context (MYR base, multi-currency operations)\n\n" +
      "The business data for this analysis period is:\n" +
      JSON.stringify(context, null, 2) +
      "\n\nWhen answering, always ground your response in the actual data provided. If data is missing or zero, say so honestly and explain what it would mean if populated.";
  }
      ...
      {tab==="insights"&&(
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <Btn onClick={()=>Object.keys(INSIGHT_PROMPTS).forEach((k,i)=>setTimeout(()=>runInsight(k),i*800))} color={P.gold}>
...
// ══════════════════════════════════════════════════════════════════
function VendorMasterModule({gf}){
  const {store,setStore}=useStore();
...
// ══════════════════════════════════════════════════════════════════
function SupplierInvoiceModule({gf}){
  const {store,setStore}=useStore();
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <SubTabs tabs={[{id:"list",label:"Invoice List"},{id:"add",label:"+ Add Invoice"}]} active={tab} onChange={setTab}/>
